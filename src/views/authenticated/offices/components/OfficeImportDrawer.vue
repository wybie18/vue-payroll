...
const downloadTemplate = () => {
  ...
  link.setAttribute('download', 'office_import_template.csv')
...
const handleParseContent = (content: string) => {
  ...
  data.forEach((row, index) => {
    ...
    if (
      index === 0 &&
      (firstCol.toLowerCase().includes('code') ||
        secondCol.toLowerCase().includes('name') ||
        firstCol.toLowerCase().includes('office'))
    ) {
      return
    }
    ...
    if (row.length < 2) {
      tempErrors.push(
        `Row ${index + 1}: Expected at least 2 columns (office code, name), got ${row.length}`,
      )
      return
    }

    const officeCode = row[0]?.trim() || ''
    const officeName = row[1]?.trim() || ''
    const abbreviation = row[2]?.trim() || ''

    if (!officeCode) {
      tempErrors.push(`Row ${index + 1}: Office Code is required`)
      return
    }

    if (!officeName) {
      tempErrors.push(`Row ${index + 1}: Office Name is required`)
      return
    }
  ...
  <Sheet :open="open" @update:open="close">
    <SheetContent class="flex flex-col sm:max-w-xl">
      <SheetHeader class="text-start">
        <SheetTitle>Import Offices</SheetTitle>
        <SheetDescription>
          Import bulk office records using a CSV file or direct text input.
        </SheetDescription>
      </SheetHeader>
        ...
        <div class="rounded-lg border border-dashed p-4 space-y-3 bg-muted/20">
          ...
          <div class="text-xs text-muted-foreground space-y-1">
            ...
            <code class="block bg-muted p-2 rounded-md font-mono text-foreground mt-1">
              office code, office name, abbreviation<br />
              OFF-001, Office of the Mayor, MAYOR<br />
              OFF-002, Human Resource Management Office, HRMO
            </code>
            <p class="text-[10px] text-muted-foreground mt-2">
              * Abbreviation is optional. Imported offices will default to "active" status.
            </p>
          </div>
        </div>
          ...
          <TabsContent value="paste" class="space-y-4 pt-4">
            <div class="grid gap-2">
              <Textarea
                id="csv-text"
                v-model="pastedText"
                placeholder="OFF-001, Office of the Mayor, MAYOR&#10;OFF-002, Human Resource Management Office, HRMO"
                rows="8"
            ...
            <div class="border rounded-md overflow-hidden max-h-48 overflow-y-auto">
              <table class="w-full text-xs text-left">
                <thead class="bg-muted text-muted-foreground uppercase font-semibold">
                  <tr>
                    <th class="px-3 py-2">Office Code</th>
                    <th class="px-3 py-2">Office Name</th>
                    <th class="px-3 py-2">Abbreviation</th>
                  </tr>
                </thead>
                <tbody class="divide-y">
                  <tr v-for="(row, idx) in parsedRows" :key="idx" class="hover:bg-muted/50">
                    <td class="px-3 py-2 font-mono font-medium">{{ row.office_code }}</td>
                    <td class="px-3 py-2">{{ row.office_name }}</td>
                    <td class="px-3 py-2 font-mono">{{ row.abbreviation || '-' }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
